...
METRIC_EXPLAINERS = {
    ...
    "Adjusted stringency-leniency index": "Actual score minus an expected score based first on the same resident, SR stage, competency, and other assessors where available.",
...
def normalize_sr_stage_value(value):
    if pd.isna(value):
        return pd.NA
    ...
    upper = text.upper()
    match = re.search(r"\b(?:SR|SENIOR RESIDENT|LEVEL|YEAR|PGY)\s*[-_ ]*(\d+)\b", upper)
    if not match:
        match = re.fullmatch(r"(\d+)(?:\.0)?", upper)
    if match:
        return f"SR{int(match.group(1))}"
    return upper


def normalize_sr_stage_series(series: pd.Series) -> pd.Series:
    return series.apply(normalize_sr_stage_value).astype("string")


def sr_stage_benchmark_score(value):
    stage = normalize_sr_stage_value(value)
    if pd.isna(stage):
        return pd.NA
    return SR_STAGE_BENCHMARK_SCORES.get(str(stage))
...
def prepare_quantitative(df_quant: pd.DataFrame, context: dict) -> pd.DataFrame:
    df = df_quant.copy()
    for col in [context["assessor_col"], context["resident_col"]]:
        if col and col in df.columns:
            df[col] = norm_text_series(df[col])
    if context["level_col"] and context["level_col"] in df.columns:
        df[context["level_col"]] = normalize_sr_stage_series(df[context["level_col"]])
...
def leave_one_assessor_expected(long_df: pd.DataFrame, keys: List[str]) -> pd.DataFrame:
    required_cols = ["Assessor", "Score"] + keys
    if any(col not in long_df.columns for col in required_cols):
        return pd.DataFrame()

    data = long_df[required_cols].copy()
    valid = data["Assessor"].notna() & data["Score"].notna()
    for key in keys:
    ...
    group_stats = (
        data.groupby(keys, dropna=False)["Score"]
        .agg(Group_sum="sum", Group_count="count")
        .reset_index()
    )
    assessor_stats = (
        data.groupby(keys + ["Assessor"], dropna=False)["Score"]
        .agg(Assessor_sum="sum", Assessor_count="count")
        .reset_index()
    )

    expected = data[["_row_id", "Assessor"] + keys].merge(group_stats, on=keys, how="left")
    expected = expected.merge(assessor_stats, on=keys + ["Assessor"], how="left")
    expected["Comparator count"] = expected["Group_count"] - expected["Assessor_count"]
    expected = expected[expected["Comparator count"].gt(0)].copy()
    expected["Expected score"] = (
        expected["Group_sum"] - expected["Assessor_sum"]
    ) / expected["Comparator count"]
    return expected[["_row_id", "Expected score", "Comparator count"]]


def fill_expected_scores(adjusted: pd.DataFrame, basis: str, keys: List[str]) -> pd.DataFrame:
    expected = leave_one_assessor_expected(adjusted, keys)
    if expected.empty:
        return adjusted

    expected = expected.set_index("_row_id")
    fill_index = adjusted.index[
        adjusted["Expected score"].isna() & adjusted.index.isin(expected.index)
    ]
    if fill_index.empty:
        return adjusted

    adjusted.loc[fill_index, "Expected score"] = expected.loc[fill_index, "Expected score"].astype(float)
    adjusted.loc[fill_index, "Comparator count"] = expected.loc[fill_index, "Comparator count"].astype(int)
    adjusted.loc[fill_index, "Expected basis"] = basis
    return adjusted


def fill_sr_stage_benchmark(adjusted: pd.DataFrame) -> pd.DataFrame:
    if "SR Stage" not in adjusted.columns:
        return adjusted

    benchmark = adjusted["SR Stage"].apply(sr_stage_benchmark_score)
    fill_mask = adjusted["Expected score"].isna() & benchmark.notna()
    adjusted.loc[fill_mask, "Expected score"] = benchmark.loc[fill_mask].astype(float)
    adjusted.loc[fill_mask, "Comparator count"] = 0
    adjusted.loc[fill_mask, "Expected basis"] = "SR stage benchmark"
    return adjusted


def expected_basis_detail(group: pd.DataFrame) -> str:
    counts = group["Expected basis"].value_counts()
    usable = counts.drop(labels=["No comparator"], errors="ignore")
    if usable.empty:
        return "No comparator"
    return "; ".join(f"{basis}: {count}" for basis, count in usable.items())
...
def adjusted_stringency_index(df: pd.DataFrame, context: dict) -> pd.DataFrame:
    ...
    extra_cols = [resident_col, level_col]
    long_df = make_long_scores(df, assessor_col, score_cols, extra_cols=extra_cols)
    if long_df.empty:
        return pd.DataFrame()

    rename_map = {}
    if resident_col and resident_col in long_df.columns:
        rename_map[resident_col] = "Resident"
    if level_col and level_col in long_df.columns:
        rename_map[level_col] = "SR Stage"
    adjusted = long_df.rename(columns=rename_map).copy()
    adjusted["Expected score"] = pd.Series(pd.NA, index=adjusted.index, dtype="Float64")
    adjusted["Comparator count"] = 0
    adjusted["Expected basis"] = "No comparator"

    has_resident = "Resident" in adjusted.columns
    has_level = "SR Stage" in adjusted.columns
    basis_order = []
    if has_resident and has_level:
        basis_order.append(("same resident + SR stage + competency", ["Resident", "SR Stage", "Competency"]))
    if has_level:
        basis_order.append(("same SR stage + competency", ["SR Stage", "Competency"]))
    if has_resident:
        basis_order.append(("same resident + competency", ["Resident", "Competency"]))

    for basis, keys in basis_order:
        adjusted = fill_expected_scores(adjusted, basis, keys)
    adjusted = fill_sr_stage_benchmark(adjusted)
    adjusted = fill_expected_scores(adjusted, "same competency", ["Competency"])
    adjusted["Adjusted difference"] = adjusted["Score"] - adjusted["Expected score"]

    comparable = adjusted.dropna(subset=["Adjusted difference"]).copy()
    if comparable.empty:
        return pd.DataFrame()

    summary = (
        comparable.groupby("Assessor")
        .agg(
            Adjusted_index=("Adjusted difference", "mean"),
            Median_difference=("Adjusted difference", "median"),
            Score_count=("Adjusted difference", "count"),
            Mean_actual_score=("Score", "mean"),
            Mean_expected_score=("Expected score", "mean"),
        )
        .round(3)
        .reset_index()
    )
    total_scores = long_df.groupby("Assessor")["Score"].count().rename("Total_valid_scores").reset_index()
    basis_details = comparable.groupby("Assessor").apply(expected_basis_detail).rename("Expected basis detail").reset_index()
    resident_comparators = (
        comparable[comparable["Expected basis"].str.contains("resident", case=False, na=False)]
        .groupby("Assessor")["Adjusted difference"]
        .count()
        .rename("Resident_comparator_scores")
        .reset_index()
    )

    summary = summary.merge(total_scores, on="Assessor", how="left")
    summary = summary.merge(basis_details, on="Assessor", how="left")
    summary = summary.merge(resident_comparators, on="Assessor", how="left")
    summary["Resident_comparator_scores"] = summary["Resident_comparator_scores"].fillna(0).astype(int)
    summary["Comparator coverage"] = (summary["Score_count"] / summary["Total_valid_scores"] * 100).round(1)
    summary = summary.sort_values("Adjusted_index", ascending=False)
    return summary
...
def report_card_pdf_lines(
    ...
    lines = [
        ...
        "Stringency compares each score with other-assessor expectations, prioritizing same resident, SR stage, and competency.",
        "",
        "Cohort comparison",
    ]
...
if not uploaded_file:
    ...
    if context["level_col"]:
        st.caption(
            f"SR stage detected from `{context['level_col']}`. "
            "Stringency comparisons use other assessors and prioritize same resident + SR stage + competency."
        )
    else:
        st.warning(
            "No SR stage column was detected. The stringency index will fall back to resident and competency comparisons, "
            "but it will not be SR-stage adjusted."
        )
    ...
    with tab_report:
        ...
        if not assessor_options:
            ...
            st.caption(
                "Comparisons are descriptive. The stringency index uses other-assessor expectations and prioritizes "
                "same resident + SR stage + competency when those comparators exist."
            )
    ...
    with tab3:
        ...
        st.caption(
            "Positive values suggest more lenient scoring; negative values suggest more stringent scoring. "
            "Expected scores are leave-one-assessor-out and use: same resident + SR stage + competency, then SR stage + competency, "
            "then resident + competency, then an SR-stage benchmark, then competency only."
        )
        if stringency.empty:
